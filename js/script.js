...
if(window.openDatabase){
    //10mb-size
    db=openDatabase("NoteApp","1.0","NotesApp Database",10000000);
    if(!db){
...
Note.prototype = {
    //fetching id , if no id set to 0 and return
    get id(){
        if (!("_id" in this))
            this._id = 0;
        return this._id;
    ...
    //set the text x passed to field
    set text(x){
        this.editField.innerHTML = x;
    },
    //fetching timestamp , if no timestamp set to 0 and return
    get timestamp(){
        if (!("_timestamp" in this))
            this._timestamp = 0;
        return this._timestamp;
    },
    //set timestamp == to passed then return, and load the date and set it.
    set timestamp(x){
